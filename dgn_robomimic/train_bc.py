...
def run(cfg: MainConfig, policy):
    ...
    for epoch in range(cfg.num_epoch):
        ...
        for step_idx in range(cfg.epoch_len):
...
def evaluate(policy, dataset: RobomimicDataset, seed, num_game):
    ...
    """
    return run_eval(
        dataset.env_params, policy, num_game=num_game, seed=seed, verbose=True
    )


def _load_model(weight_file, env: PixelRobosuite, device, cfg: Optional[MainConfig] = None):
    if cfg is None:
        #cfg_path = "/iris/u/aml2023/rl_refinement/ibrl/exps/bc_sweep5/square_50demos_seed1_no_drop/cfg.yaml"
    ...
    #weight_file = '/iris/u/aml2023/rl_refinement/ibrl/exps/bc_sweep5/square_50demos_seed1_no_drop/model0.pt'
