...
from rlpd.explore_utils import make_state_exploration_module
...
flags.DEFINE_integer("cov_size", 256, "cov batch size.")
flags.DEFINE_integer("schedule", 0, "cov batch size.")
flags.DEFINE_integer("cov_start", 50, "cov batch size.")
flags.DEFINE_integer("cov_train_epochs", 100, "cov train.")
flags.DEFINE_integer("update_freq", 2000, "cov batch size.")
flags.DEFINE_integer("layer_norm", 0, "cov batch size.")
...
def main(_):
    ...
    log_dir = os.path.join("/iliad/u/perryd/rl_refinement/rlpd", exp_prefix)
    ...
    explore_module = make_state_exploration_module(seed=FLAGS.seed, layer_norm=FLAGS.layer_norm, cov_train_epochs=FLAGS.cov_train_epochs, cov_size=FLAGS.cov_size, update_freq=FLAGS.update_freq, state_dim=env.observation_space.shape[0], action_dim=env.action_space.shape[0])

    explore_module.set_dataset(ds)
    ...
    for i in tqdm.tqdm(range(FLAGS.cov_start // FLAGS.cov_train_epochs)):
        explore_module._update(agent, log_step=0)
    ...
    for i in tqdm.tqdm(
        ...
        explore_module.update_exploration(agent, step=i)
        ...
        if i < FLAGS.start_training:
            ...
            pert = explore_module.get_pert(np.expand_dims(observation, 0))
            pert = np.squeeze(pert)


            if FLAGS.schedule > 0:
                pert = pert * np.exp(-(max(i, 1)/FLAGS.schedule))
