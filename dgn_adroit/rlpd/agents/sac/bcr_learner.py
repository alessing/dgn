...
class BCRLearner(Agent):
    ...
    def update_actor(self, batch: DatasetDict) -> Tuple[Agent, Dict[str, float]]:
        ...
        def actor_loss_fn(actor_params) -> Tuple[jnp.ndarray, Dict[str, float]]:
            ...
            batch_log_probs = jnp.clip(batch_log_probs, -100, 0)
            ...
            q = qs.mean(axis=0)
            actor_loss = (
                log_probs * jnp.clip(self.temp.apply_fn({"params": self.temp.params}), 0.01, 10.0) - q - batch_log_probs * self.bc_loss
            ).mean()
            return actor_loss, {"actor_loss": actor_loss, "entropy": -log_probs.mean()}
    ...
    @partial(jax.jit, static_argnames="utd_ratio")
    def update_bc(self, batch: DatasetDict, utd_ratio: int):
        ...
        new_agent, actor_info = new_agent.update_actor_bc(mini_batch)
        # new_agent, temp_info = new_agent.update_temperature(actor_info["entropy"])
